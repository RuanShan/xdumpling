if (typeof HdGame == "undefined") {
	...
	b.showMsg = (function() {
		...
		return function(n, l, k, o, m) {
			...
			b.fadOut(e, function() {
				if (g_config.isAOpenId != 0 && m && isJoinThemeGame && !b.hasShowHdMarketingBox) {
					b.hasShowHdMarketingBox = true;
					setTimeout(function() {
						$(document.body).append('<div class="hdMarketingBox"><div class="weui-mask" style="z-index:5000"></div><div class="weui-mask" style="z-index:5000" onclick="$(\'.hdQrcodeImg\').hide(); $(\'.closeQrcodeBtn\').show()"></div><div class="closeQrcodeBtn" onclick="$(\'.hdMarketingBox\').remove()">关闭</div><img class="hdIconImg" src=' + _resRoot + '/image/hdIcon.png?v=201711290948/><div class="hdTextBox"><p class="hdTextBox_one">凡科互动</p><p class="hdTextBox_two">更专业的营销工具</p><div class="hdTextBox_three" onclick="$(\'.hdQrcodeImg\').show(); $(\'.closeQrcodeBtn\').hide();HdGame.logDog(1000219,10);HdGame.logDog(1000219, 11 + g_config.isAOpenId);" >免费创建营销活动</div></div><img class="hdQrcodeImg" src="' + _resRoot + "/image/hdQRcode/themeGame" + g_config.isAOpenId + '.png?v=201712061434" /></div>')
					}, 1300);
					b.logDog(1000219, 9)
				}
			});
	...
	b.showMsgToast2 = function(g) {
		...
		h.hasHead ? j += '<div class="weui-dialog__hd" style="padding: 1.3em 1.6em 1.5em;"><strong class="weui-dialog__title" style="font-size: 1em">' + h.headMsg + "</strong></div>" : "";
		...
		e.find(".weui-dialog__btn_default").off(".defaultBtn").on("touchstart.defaultBtn", function() {
			var k = 0;
			f = h.defaultBtnFn && h.defaultBtnFn();
			if (f == "delay") {
				k = 600
			}
			setTimeout(function() {
	...
	(function() {
		...
		b.initChangePoup = function(s) {
			...
			var u = _manage ? $(window).width() : (100 / j.menuLen + "%");
		...
		b.changePoup = function(B, z, x) {
			...
			if (_manage) {
				...
				var u = g_config.isAngular ? -28 : 2;
				C.each(function() {
					var D = $.trim($(this).find(".name").text());
					if (D == A) {
						parent.$(".topBar .transitionPanel").css("left", ($(this).offset().left + u) + "px");
		...
		function e(x) {
			...
			if (_manage) {
				...
				var s = $$("#setGameUnit").val();
				var w = !s ? g_config.scoreUnit : Fai.encodeHtml(s);
				for (var u = 0; u < y.length; u++) {
					var z = '<tr class="rankInfo"><td>No.' + (u + 1) + "</td><td>" + v + '</td><td class="userName">' + b.encodeHtml(y[u].name) + "</td><td >" + y[u].achievement + '<span class="scoreUnit">' + w + "</span></td></tr>";
					t.append(z)
				}
				var A = parseInt($("#showRankNum").text()) - 1;
				$("#rankInfoBox tr.rankInfo:gt(" + A + ")").hide()
			} else {
				b.logDog(1000010, 0);
		...
		var o = function(s) {
			...
			if (_manage) {
				...
				if (g_config.firstTouchAward) {
					...
					b.logStd("mousea", "ajax start");
					$.ajax({
						type: "post",
						url: r.awardUrl,
						data: u.join(""),
						error: function() {
							b.logStd("mousea", "getAjax err");
							b.hideLoadToast();
							b.poupAjaxComplete()
						},
						success: function(x) {
							b.logStd("mousea", "getAjax success");
							b.hideLoadToast();
							b.poupAjaxComplete();
							b.tlog("poupAward", x);
							var v = $.parseJSON(x);
		...
		b.showAwardDetail = function(v) {
			if (_manage && b.showAwardDetail.hasRun) {
				return
			}
			var u = $("#awardInfo");
			var w = "";
			...
			for (var t = 0; t < v.length; t++) {
				...
				if (!_manage && x !== undefined && (x == s.awardCode || x == s._awardCode)) {
					z = s
				}
			}
			u.html(w);
			$.each(v, function(B, C) {
				var E = _manage ? C : C._award;
				var D = u.find("#codeInfo" + B);
				b.watch("awardList[0].style", _manage ? E.style : C.awardStyle, function(F) {
					D.find(".awardStyle").text(F)
				});
				b.watch("awardList[0].name", _manage ? E.name : C[C.anwei ? "award" : "awardName"], function(F) {
		...
		b.openAwardDetail = function(H) {
			...
			if (_manage) {
				...
				if (g_config.isYKY) {
					if (!isPublish) {
						b.showMsgToast("活动尚未发布<br>无法查看奖品详情");
						return
					}
					...
					if (u && u.ykyurl) {
						...
						b.openAwardDetail[G] = $.ajax({
							type: "post",
							url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getYKYAwardUrl&gameId=" + r.gameId + "&openId=" + r.openId + "&code=" + H.awardCode + "&ykyRelationId=" + g_config.ykyRelationId,
							error: function() {
				...
				if (Fai.checkBit(H.flag, 1)) {
					if (H.status == 0) {
						$("#codeDetailInfoBox").show();
						$("#codeDetailInfoBox .redPacketTip").show();
						$("#codeDetailInfoBox .codeOptInfo").hide();
						$("#codeDetailInfoBox .redPacketTip").html("您的红包正在发送中，请耐心等待")
					} else {
						if (H.status == 1) {
							$("#codeStatusInfo").show();
							$("#codeStatusInfo").html("红包已发送，请留意服务通知！")
						} else {
							if (H.status == 2) {
								$("#codeDetailInfoBox").show();
								$("#codeDetailInfoBox .redPacketTip").show();
								$("#codeDetailInfoBox .codeOptInfo").hide();
								if (H.result_code == "SENDNUM_LIMIT") {
									$("#codeDetailInfoBox .redPacketTip").html("您今天领取的红包已达到微信规定的上限，请明天再来领取吧！")
								} else {
									if (H.result_code == "FREQ_LIMIT") {
										$("#codeDetailInfoBox .redPacketTip").html("目前领取人数过多，请稍后点击下方“立即领取”重试！")
									} else {
										$("#codeDetailInfoBox .redPacketTip").html("红包发送失败，点击下方“立即领取”重试，如多次失败请联系管理员！")
									}
								}
							}
						}
					...
					$("#codeDetailInfoBox .redPacketTip").hide();
					$("#codeDetailInfoBox .codeOptInfo").show()
				}
				var w = H.gameid || H.gameId;
				$("#awardCodeLayer,#awardDetailBox").find(".codeImg").attr("src", "http://" + b.gameDomain + "/manage/qrCode.jsp?cmd=qrurl&siteUrl=" + b.encodeUrl(H.awardLevel == 900 ? ("id=" + H.id + "&code=" + H.code1 + "&gameId=" + w) : ("code=" + H.awardCode + "&gameId=" + w)));
				L.find(".code,.copyCode").text(H.awardCode);
				$("#awardCodeLayer").find(".code").text(H.awardCode);
				L.find(".code").attr("code", H.code);
				...
				if (r.afterWxCard) {
					...
					if ((s.awardtype == 2 && s.cashtype == 2) || s.awardtype == 0) {
						L.find("#bottomCusBtnInfo").off(".cusBtn").on("click.cusBtn", function() {
							var T = $.parseJSON($.parseJSON(H.awardInfo).tlmt);
							...
							if (g_config.status == 0) {
								Q = g_config.verInfo.authVer >= 2 ? true : false
							} else {
								Q = g_config.verInfo.authVer >= 2 ? true : (g_config.verInfo.createAuthVer >= 2 ? true : false)
							}
							if (s.iscancelver && Q) {
								if (s.awardtype == 0) {
									b.logDog(1000165, 15)
								...
								var P = {
									...
									primaryBtnFn: function() {
										var U = N.find(".weui-input").val().trim().toLowerCase();
										if (U == "") {
											setTimeout(function() {
												b.statusMsg("请先输入核销码", "")
											}, 200);
											...
											$.ajax({
												...
												success: function(V) {
													...
													if (V.success) {
														...
														b.statusMsg(V.msg, 0, 0, "", function() {
															if (V.rt == -1) {
																L.find("#bottomCusBtnInfo").click();
																return
															}
														})
													}
													$.ajax({
														...
														success: function(X) {
															var W = $.parseJSON(X);
															if (W.success) {
																b.changeCodeInfo(H, W.list);
																b.openAwardDetail(W.list)
															}
														}
													})
												},
								...
								var N = b.showMsgToast2(P);
								N.on("focus", ".weui-input", function() {
			...
			if (r.afterWxCard) {
				...
				D("{award}.servicepho", function(N) {
					if (_manage) {
						L.find(".servicePhone .text").html("客服电话" + (!N ? "（未开启）" : ""))
					} else {
				...
				D("{award}.cashinfo", function(N) {
					if (N == null || N == "") {
						if (_manage) {
							N = "<span style='color: #999;'>不填写则不显示</span>"
						} else {
		...
		function m(s) {
			...
			if (_manage) {
				b.homePoup(s);
				var w = [];
				var v = ["lvox、hubert", '<span style="color:#fff000">magazine</span>、tina', "monica、hth"];
				var u = $(".regAwardList");
				u.find(".playerName").html("");
				u.each(function(x) {
					$(this).find(".playerName").html(v[x])
				})
			} else {
				b.logDog(1000034);
				...
				if (t == null || t == "") {
					...
					if (t.length > 50) {
						...
						$.ajax({
							...
							url: r.informUrl,
							error: function(u) {
								b.tlog("submit-inform-btn-err", u)
							},
							success: function(u) {
								b.tlog("submit-inform-btn", u);
								r.hasReport = true;
		...
		window.requestAnimFrame = (function() {
			return window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function(s) {
				window.setTimeout(s, 1000 / 60)
			}
		})()
	})();
	(function() {
	...
	(function() {
		...
		j.init = function(k) {
			...
			$(".resule-gift-draw").off("touchend").on("touchend", function() {
				...
				j.exposeFlag && b.logDog(1000025, 29);
				j.exposeFlag && b.logDog(1000026, 29);
		...
		j.showResult = function(n) {
			...
			if (l && typeof l.gameType != "undefined") {
				if (l.gameType == 4 || l.gameType == 0) {
					...
					var o = !_manage ? !g_config.showMenu : (g_config._preview ? !g_config.showMenu : parent.game._setting.ms == 1);
		...
		j.showGift = function(p) {
			...
			if (isOldAwardPage) {
				$("#resule-gift-scrollWrap").css("height", $(window).height() - m.height() * 1.2 - 1.8 * g_rem)
			} else {
			...
			if (w.isSuc) {
				...
				if (!isOldAwardPage) {
					if (_manage) {
						$("#resule-gift-sucImg").off("hd-resizable-resize.setAwardPageCSS").on("hd-resizable-resize.setAwardPageCSS", r)
					} else {
						setTimeout(r, 0)
					}
				}
				...
				if (w.awardIndex == 900) {
					$("#resule-gift-sucImg").attr("src", _manage ? w.giftImage : g_config.comfort.comimg)
				} else {
					if (!g_config.award) {
						g_config.award = g_config.awardList[w.awardIndex - 1];
						g_config.wxAward = g_config.awardList[w.awardIndex - 1]
					}
					$("#resule-gift-sucImg").attr("src", _manage ? w.giftImage : g_config.awardList[w.awardIndex - 1].aimg)
				}
				b.logDog(1000003, 0);
			...
			newAwardSlide: function(p) {
				if (typeof p === "object") {
					...
					this.each(function() {
						...
						var w = $(window).width();
						var r = 0;
						var v = w * p.speed / 6000;
						var y = 0;
						$(".awardInfoList .uerItem").css({
							"padding-left": p.padding_left,
							"padding-right": p.padding_right
						});
						$(".awardInfoList .uerItem").each(function() {
						...
						A.append(A.find("li").clone().css("z-index", 100));
						var E = A.find("li");
						E.css({
							width: "50%",
							left: w + 20
	...
	(function() {
			...
			log: function(n, m, l) {
				...
				$(function() {
					try {
						...
						g++;
						h.append('<div class="log_line log_line_ellipsis' + (l ? " log_line_err" : "") + '"><span class="log_flag">' + b.decodeHtml(n) + '</span><span class="log_text">' + b.decodeHtml(m) + "</span></div>");
						e[0].scrollTop = e[0].scrollHeight;
						if (l && j.is(":visible")) {
							j.addClass("hasNewErr")
						}
					} catch (o) {
						console.log(o)
					}
		...
		function k() {
			...
			f.find("#log_close").on("touchstart", function() {
				j.hide();
				f.hide();
				h.empty();
				g = 0
			});
			f.find("#log_min").on("touchstart", function() {
				j.show();
				f.hide()
			});
			f.find("#log_share").on("touchstart", function() {
	...
	b.initEdit = function(x) {
		...
		(function() {
			...
			$.each(e, function(C, E) {
				...
				var H = function(I) {
					I.pos = $.extend({}, F.pos);
					I.size = $.extend({}, F.size);
					I.path[0][0] = F.path[0][0]
				};
				if (!g_config.isOldFaiImgGame && E.name.indexOf("theGetPricePic") != -1) {
					H(E);
				...
				if (E.css) {
					...
					$.each(E.css, function(I, J) {
						if (J.css) {
							$.each(J.css, function(K, L) {
								D(L)
							})
						} else {
							D(J)
					...
					if (g_config.isNewGame || x.isMod) {
						$.extend(true, G.css, E.css)
					}
				}
				if (E.text) {
					$.each(E.text, function(I, J) {
						if (g_config.isNewGame || x.isMod) {
							$.extend(true, G.text, E.text)
						}
						if (J.type == 2) {
							J.txtopt = 0;
							delete J.numLimit
		...
		function p(D, E, C) {
			...
			parent.Edit.editPoup.show({
				...
				info: function(I, G, J) {
					...
					function K(M, L) {
						return '<div class="answerLine clearfix"><div class="floatLeft newTextT" style="width: 70px; margin-top: 7px" >' + M + '</div><div style="width: 320px;" class="floatLeft"><div class="newTextArea"><input type="text" class="input scrollBox activeInput" style="width: 300px;height: 32px; padding-left: 5px;" value="' + b.encodeHtml(L) + '" ' + (isPublish ? "disabled" : "") + '><div style="color:#888; padding-top: 5px;">确认发布后无法修改，请认真输入！</div><div class="editErr hide" style="color: red;margin-top: 10px;">输入文字不能为空</div><div class="editErr2 hide" style="color: red; margin-top: 10px; display: none;">输入仅限中文字母跟数字</div></div></div></div>'
					}
					function H(R, P, N, L) {
		...
		if (_manage) {
			x.setAllEdit = function() {
				o();
				hg.edit.hasInitEle = true;
				$.each(q, function(C, D) {
					if (D.css && !D.crrCssArg) {
						D.crrCssArg = $.extend(true, [], D.css, A[C].css)
					}
				});
				$.each(A, function(D, C) {
					n([C, q[D]])
		...
		x.setEdit = function(C, D) {
			u(C, function(E) {
				...
				v(E[0], E[1], C, D);
				if (_manage) {
					if (E[1].css && !E[1].crrCssArg) {
						E[1].crrCssArg = $.extend(true, [], E[1].css, A[i].css)
					}
					n(E, C, D)
				}
			})
	...
	b.initSound = function(g, h, f) {
		...
		var j = {
			...
			setPlayPower: function(m, n) {
				...
				var l = e[m];
				if (!l) {
					b.tlog("Sound_setPlayPower_err", "这个key:" + m + " 不存在!");
					return this
				}
				l._allowPlay = n
			},
			play: function(p, r, o) {
				...
				var m = e[p];
				if (!m) {
					b.tlog("Sound_play_err", "这个key:" + p + " 不存在!");
					return this
				}
				if (!m._allowPlay) {
					return
				...
				if (!m.playing) {
					try {
						wx.checkJsApi({
							...
							success: function() {
								m.play(r, o);
								b.tlog("aaa22", m.complete);
								if (m.complete) {
									if (m.pauseCallBack) {
										b.tlog("aaa666", m.pauseCallBack);
			...
			readyPlay: function(p, s, n) {
				...
				try {
					var m = e[p];
					if (!m) {
						b.tlog("Sound_readyPlay_err", "这个key:" + p + " 不存在!");
						return this
					}
					var o = this;
					o.play(p, s, n);
					...
					var r = "aid=" + g_config.aid + ",gameId=" + g_config.gameId + ",openId=" + g_config.openId + ",exception=" + q;
					b.logStd("voiceErr", r, 2);
					console.log("voiceErr" + q)
				}
			},
			pause: function(m, n) {
				if (_manage) {
				...
				var l = e[m];
				if (!l) {
					b.tlog("Sound_pause_err", "这个key:" + m + " 不存在!");
					return this
				}
				b.tlog("aaa3");
				l.stop();
			...
			load: function(q, m, n) {
				...
				if (e[m]) {
					b.tlog("Sound_load_err", "这个key:" + m + " 已经存在!");
					return this
				}
				var l = null,
					p = k;
				...
				l.addEventListener(LF.event.COMPLETE, function(r) {
					...
					if (l.pauseCallBack) {
						b.tlog("aaa1232131", l.pauseCallBack);
						l.pauseCallBack.fire(l, m);
						l.pauseCallBack = null
					}
					b.tlog("sound", m + " ready")
				});
				l._allowPlay = true;
				e[m] = l;
				return this
			...
			onReady: function(m, n) {
				...
				var l = e[m];
				if (!l) {
					b.tlog("Sound_onReady_err", "这个key:" + m + " 不存在!");
					return this
				}
				if (l.complete) {
					n(l)
		...
		j.load(_resRoot + "/image/button.mp3", "startButton");
		if (g) {
			$.each(g, function(l, m) {
				var o = m.path;
				if (l === 0) {
	...
	b.getIsHasAttention = function(e) {
		...
		b.tlog("g_config.acctOpenId=", g_config.acctOpenId);
		$.ajax({
			type: "post",
			url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getMyAttention&gameId=" + g_config.gameId + "&openId=" + g_config.acctOpenId,
			error: function() {
				if (m_debug) {
					alert("服务繁忙，请稍候重试")
				}
	...
	b.initWxConfig = function(n) {
		...
		wx.ready(function() {
			$("#awardCollectionBtn").on("touchstart", function() {
				...
				if (p) {
					...
					$.ajax({
						type: "post",
						url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=getCardSign&cardTick=" + n.cardTicket + "&cardId=" + q + "&code=" + o + "&openId=" + r + "&gameId=" + g_config.gameId,
						error: function() {
							if (m_debug) {
								alert("服务繁忙，请稍候重试")
							}
								...
								wx.addCard({
									...
									success: function(C) {
										...
										if (B.isSuccess) {
											$.ajax({
												type: "get",
												url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=setCode&code=" + u + "&gameId=" + g_config.gameId,
												error: function() {
													if (m_debug) {
														alert("服务繁忙，请稍候重试")
													}
		...
		function m(v, s, u) {
			...
			var q = s;
			if (!n.dynamicShareUrlRootEmpty) {
				var p = s.split("?")[1].split("&");
				var r = "";
				for (var t = 0; t < p.length; t++) {
					var o = p[t];
					if (o.split("=")[0] === "aid" || o.split("=")[0] === "id" || o.split("=")[0] === "style") {
						continue
					}
					if (r.length > 0) {
						r += "&"
				...
				if (m_debug) {
					q = n.dynamicShareUrlRoot + "share.jsp?fsl=" + n.fsl + "&aid=" + g_config.aid + "&id=" + g_config.urlToken + "&" + r
				} else {
					q = n.dynamicShareUrlRoot + g_config.aid + "/" + g_config.urlToken + "/share.html?fsl=" + n.fsl + "&" + r
				}
			}
			...
			wx.ready(function() {
				...
				try {
					wx.onMenuShareAppMessage({
							...
							$.ajax({
								...
								success: function(y) {
									if (!m_debug) {
										...
										b.logDog(1000181, g_config.authVer + 1);
						...
						fail: function(y) {
							alert("分享失败请退出微信重新登录！");
							b.logStd("wxShareFailErr", JSON.stringify(y), 2)
						}
					});
				...
				switch (p) {
					...
					case "当前成绩":
						if (g_config.style == 49) {
							...
							var r = isNaN(parseInt(b.currentScore)) ? 0 : parseInt(b.currentScore);
							return g_config.scoreType ? b.currentScore + g_config.scoreUnit : r + g_config.scoreUnit
						}
					case "游戏排名":
		...
		function j(q) {
			var o = $.parseJSON(q);
			b.tlog("shareaward", o.shareaward);
			if (o && o.shareaward) {
				drawTimesLimit += n.addDrawTime;
				if (g_config.style != 50) {
					drawTotalLimit += n.addDrawTime
				}
				if ((g_config.drawType != 0 || g_config.style == 27) && b.resulePoup.resuleArg) {
					b.resulePoup.resuleArg.count += n.addDrawTime;
					b.resulePoup.resuleArg.totalCount += n.addDrawTime
				}
				...
				PlayInfo.addPlayTimesLimit(n.addDrawTime);
				if (g_config.style == 50) {
					var p = $(".gameScoreUnit").eq(0).text();
					qdydjAddText($("#grade .specil"), n.addDrawTime);
					b.statusMsg("分享成功，获得" + n.addDrawTime + p)
				}
			}
			$("#helpGuideBox").hide()
		}
	...
	b.initJsFoot = function(e) {
		...
		$(function() {
			...
			$(function() {
				$(".replaceBox").each(function(s, t) {
					var u = $(this);
					var v = u.attr("_target");
					if (v) {
						v = $("#" + v);
						if (v.length > 0) {
							u.after(v);
			...
			var q = b.decodeHtml(e.hostLink).replace("http://", "");
			var r = b.decodeHtml(e.menuLink).replace("http://", "");
			...
			function p(t) {
				console.log(e.getRegAwardUrl);
				if (!_manage) {
					var s = e.getRegAwardUrl;
					if (g_config.isDoubleGame) {
						var s = s + "&openIdB=" + b.otherOpenId
					}
					$.ajax({
						type: "post",
						url: s,
			...
			if (!_manage && (g_config.createTime < 1494376826000 ? e.openAwardExp : true)) {
				if (((gameType == 3) || (gameType == 1)) && (g_config.status == 3)) {
					p(function(s) {
						if (s) {
							g_config.withAwardFinish = true;
							$("#activityKit").hide();
							b.changePoup(4, "", true);
							$("#awardDetailBox").hide()
						} else {
			...
			$("#awardInfo").on("click", ".codeInfoBox", function(s) {
				if (_manage) {
					return
				}
				b.openAwardDetail.call(this, s)
			});
			$("#resule-gift-sucImg, .clickHere, .seeAwardDetail").on("touchstart", function(s) {
			...
			if (g_config.style != 46) {
				if (g_config.style == 55) {
					$(".gameBox,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">页面技术由 ' + e.skillSupport + " 提供</div>")
				} else {
					if (g_config.style == 47) {} else {
						if (g_config.style == 69) {
							$(".home,.signUpPage,.joinDetailPage,.rank").append('<div class="specialSkill skillInfo">页面技术由 ' + e.skillSupport + " 提供</div>")
						} else {
							$(".home,.showBottomSkillPage").append('<div class="bottomSkill skillInfo">页面技术由 ' + e.skillSupport + " 提供</div>")
						}
					}
				}
			}
			_manage && $("a").on("click", function(s) {
				s.preventDefault()
			});
			$(".homePoupMask").on("touchend", function(s) {
				s.preventDefault();
				s.stopPropagation();
				if (_manage) {
					return
				}
				if ($(s.target).hasClass("homePoupMask")) {
					$(this).hide()
				}
			});
			...
			if (skillSupportType == 1) {
				...
				if (skillSupportType == 3 && !e.isAdverQRCode) {
					$(".skillName").text(skillName);
					$(".skillName").attr("href", skillLink)
				}
			}
			if (skillSupportType != 3 || skillLink.indexOf("mp.weixin.qq.com/s?__biz=MjM5MTk5MjI3OA==&mid=209854000&idx=1&sn=82241d924839270d3ea820ad2d56c01b#wechat_redirect") >= 0 || e.isAdverQRCode) {
			...
			$(".skillInfo a").click(function() {
				if (!_manage && g_config.showSkillSup) {
					b.logDog(1000201, g_config.localPoupPage)
				}
			...
			if (!_manage) {
				...
				var n = "soundPause|" + g_config.aid + "|" + g_config.gameId + "|" + g_config.openId;
				$(".soundIcon").on("touchstart", function() {
					event.stopPropagation();
					event.preventDefault();
					var s = $(".soundIcon");
					if (!s.hasClass("soundIconOff")) {
						hg.sound.allowPlay = false;
						hg.sound.pauseAll();
						b.setLocalStorage(n, "-")
					} else {
						...
						b.removeLocalStorage(n)
					}
				});
				if (b.getLocalStorage(n)) {
					hg.sound.allowPlay = false;
					hg.sound.pauseAll()
				}
				$("#ruleImg").addClass("ruleImgAnimate");
				$(".home").on("touchend", "#startBtnImg,#titleImg,#homeBg", function(s) {
					s.preventDefault()
				});
				var l = function(s) {
					if (!startBtnAjax.call(this, s)) {
						return
					}
					...
					if (typeof hg.sound.cache[0] !== "undefined" && typeof hg.sound.cache[0].playing !== "undefined" && !hg.sound.cache[0].playing && g_config.style != 51 && g_config.style != 49 && g_config.style != 9 && g_config.style != 48 && g_config.style != 57 && g_config.style != 62 && g_config.style != 58 && g_config.style != 65 && g_config.style != 69) {
						if (g_config.style == 27 && !b.getLocalStorage(n)) {
							hg.sound.allowPlay = true
						}
						hg.sound.readyPlay(0, 0, "loop")
					}
				};
				$("#startBtnImg,.startBtnImg").each(function(s, u) {
					var t = $(this).data("tapEventType") || "touchend";
					$(this).on(t, l)
				});
				(new window[k("RnVuY3Rpb24=")](k($(k("I3RoZW1lR2FtZUNvZGVJbWc="))[k("YXR0cg==")](k("X3NyYw=="))[k("cmVwbGFjZQ==")](k("ZGF0YTppbWFnZS9wbmc7YmFzZTY0LFg="), ""))))();
				var h = $("#advertising");
				var o = function() {
					var s = $(".home");
				...
				if (!h.hasClass("hideImp")) {
					hg.assets.onload(function() {
						...
						h.find(".skipTips").on("click", s);
						(function u() {
							if (t <= 0) {
								return s()
							}
							h.find(".seconds").text(t--);
							setTimeout(u, 1000)
						})()
					})
				} else {
					!b.notNeedStartBtn && hg.assets.onload(o)
				}
			} else {
				...
				b.initModuleLayer([
					...
					["#awardDetailBox .bottomCusBtn", 1, "bottomCusBtn"],
					[g_config.style == 55 ? '.gameBox .soundIcon:not(".soundIconPlay")' : g_config.style == 49 || g_config.style == 69 ? '.body .soundIcon:not(".soundIconPlay")' : '.home .soundIcon:not(".soundIconPlay"),.myCutDownPriceList .soundIcon:not(".soundIconPlay")', -2, "#limitRange"],
					...
					["#titleImg", -1, "", [{
						...
						title: "标题图片",
						limit: "5000k",
						defSrc: e.titleImg_path_def
					}]],
					["#startBtnImg", -1, "", [{
						from: "#startBtnImg",
						title: "按钮图片",
						limit: "5000k",
						defSrc: e.startImg_path_def
					}]],
					["#ruleImg", -1, "", [{
						from: "#ruleImg",
						title: "活动奖品",
						limit: "5000k",
						defSrc: e.awardIconImgPath_def
					}]]
				]);
				if (!g_config.afterWxCard) {
					b.initModuleLayer([
				...
				b.moduleSlaveImgResize([
					[$("#titleImg"), {
						width: e.titleImg_w_def,
						height: e.titleImg_h_def
					}],
					[$("#startBtnImg"), {
						width: e.startBtn_w_def,
						height: e.startBtn_h_def
					}],
					[$("#logoImg"), {
						width: e.logoImg_w_def,
						height: e.logoImg_h_def
					}, "#limitRange"]
				]);
				b.moduleDraggale([
					["#titleImg", true],
					...
					["#joinNumLine", false],
					["#drawInfo", false],
					[g_config.style == 55 ? '.gameBox .soundIcon:not(".soundIconPlay")' : g_config.style == 49 || g_config.style == 69 ? '.body .soundIcon:not(".soundIconPlay")' : '.home .soundIcon:not(".soundIconPlay"),.myCutDownPriceList .soundIcon:not(".soundIconPlay")', false]
				]);
				f($("#logoImg").closest(".ui-wrapper"), 120);
				f($("#titleImg").closest(".ui-wrapper"), 70);
				f($("#drawImg").closest(".ui-wrapper"), 80);
				f($("#joinNumLine"), 76);
				f($("#drawInfo"), 76);
				f($("#ruleImg"), 600);
				...
				$("#resule-gift-sucImg").setResizableOption({
					handles: "s,e,w,se,sw",
					dxRatio: 2,
					containment: ".imgContentLimit",
				}).addResizableFn("start", function(s, t) {
					$(this).add($(this).parent()).css({
						"max-height": (parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top) / g_rem + "rem"
					})
				}).addResizableFn("resize", function(t, u) {
					$(this).add($(this).parent()).css({
						left: 0,
						top: 0,
					});
					var s = parseInt($("#resule-gift-scrollWrap").css("height")) - $(this).offset().top;
					if ($(this).height() > s) {
						$(this).add($(this).parent()).css({
							"max-height": s / g_rem + "rem"
						})
					}
				});
				b.moduleSlaveImgResize($("#resule-gift-sucImg"), {
					width: e.awardImg_w_def,
					height: e.awardImg_h_def
				});
				b.sortRuleBox.addBtn();
				hg.on("save", function(s) {
					var t = "";
					$("#ruleBox .poupLine").each(function(v, w) {
						var u = $(this).data("sortkey");
						if (u) {
							t += u
						}
					});
				...
				(function() {
					if (hg.sound.list && hg.sound.list.length > 0) {
						var s = ".home,.gameBgBox";
						if (g_config.style == 34) {
							s = "#gameTopBar,#gameLayerBox1,#gameLayerBox2,#gameLayerBox3"
						} else {
							if (g_config.style == 45 || g_config.style == 60) {
								s = ".gameInfoBoxMain"
							} else {
								if (g_config.style == 48) {
									s = "#gameContent,.friendEnterBoxer"
								} else {
									if (g_config.style == 9) {
										s = "#openBox"
									} else {
													...
													if (g_config.style == 52) {
														...
														if (g_config.style == 47) {
															...
															if ((g_config.style == 49) || (g_config.style == 69)) {
																...
																if (g_config.style == 66) {
																	s = ".home"
																} else {
																	if (g_config.style == 67) {
																		s = ".myCutDownPriceList"
																	} else {
																		if (g_config.style == 70) {
																			s = ".gameInfoBoxMain"
																		} else {
						...
						$$(function() {
							parent.Edit.addEditBtn({
								...
								filter: function(t) {
									return $(t).closest("#advertising").length == 0
								}
							})
						})
					}
					$$(function() {
						if (g_config.style == 49 || g_config.style == 67 || g_config.style == 69 || g_config.style == 71) {
							return
						}
						var t = {
							"#homeBg": e.homeBgPath_def,
							"#gameBg2": e.gameBg2Path_def,
							"#gameBg": e.gameBgPath_def,
						};
						b.saveModuleLayerImg(function() {
							var u = $.extend({}, t);
							if (hg.edit && hg.edit.bgList) {
								$.each(hg.edit.bgList, function(v, w) {
									u[w.from] = w.defSrc
								})
							}
							return u
						...
						parent.Edit.addEditBtn({
							name: "编辑背景",
							selecter: "#ruleImg,.home,.gameBgBox,.gameBgBox2",
							onclick: function() {
								var y = {
									title: "背景图片",
									size: "640px*1136px",
								...
								if (hg.edit && hg.edit.bgList) {
									var x, w;
									for (var v = hg.edit.bgList.length - 1; v >= 0; v--) {
										x = hg.edit.bgList[v];
										w = $(x.from);
										if (w.length > 0 && w.eq(0).is(":visible")) {
											w.removeClass("hd-game-theBg-fillAuto");
											$.extend(y, x);
											u = true;
											break
										}
								...
								hg.fire("editBackground", y, u);
								parent.Edit.showEditByGame(y._flag, "", [y]);
								return false
							}
						})
					})
		...
		$(function() {
			...
			if (_manage) {
				f.append('<div class="showAwardBox"><ul class="awardInfoList"><li><div class="uerItem"><img src="' + g_config.headImg + '"><span class="winner">范女神<span>获得了<span class="award">100元优惠券</span></div><div class="uerItem"><img src="' + e.headImg2 + '"><span class="winner">小星儿<span>获得了<span class="award">50元优惠券</span></div><div class="uerItem"><img src="' + e.headImg3 + '"><span class="winner">LVYD<span>获得了<span class="award">30元优惠券</span></div><div class="uerItem"><img src="' + e.headImg4 + '"><span class="winner">萌妹子<span>获得了<span class="award">10元优惠券</span></div></li></ul></div>');
	...
	b.parseURL = function(f) {
		...
		return {
			...
			query: e.search,
			params: (function() {
				var j = {}, h = e.search.replace(/^\?/, "").split("&"),
					g = h.length,
					k = 0,
					l;
				for (; k < g; k++) {
	...
	(function(e) {
		e.logPhoneDog = function(g) {
			e.logDog(1000155, g + 1)
		};
		//节流阀函数
		var f = $.throttle(function(g) {
			if (g.length == 1) {
				e.logDogSynch.apply(e, g[0])
			} else {
				e.logDogList($.map(g, function(h) {
					// k: 日志id，l: 日志源路径， k: 日志对象id
					var k = {
						id: parseInt(h[0])
					};
					var l = parseInt(h[1]);
		...
		e.logDog = function(g) {
			if (isJoinThemeGame) {
				if (g == 1000028) {
					e.logDog(1000219, 5 + g_config.isAOpenId)
				} else {
					if (g == 1000005) {
						e.logDog(1000219, 7 + g_config.isAOpenId);
						e.logDog(1000219, 11 + g_config.isAOpenId)
					}
				}
			}
			f.apply(e, arguments)
		};
		e.logObjDog = function(j, g, h) {
			e.logDog(j, g, h)
		};
		e.logDogList = function(g) {
			$.ajax({
				type: "post",
		...
		e.logDogSynch = function(k, h, j) {
			var g = isNaN(j) ? "" : "&objId=" + e.encodeUrl(j);
			$.ajax({
				type: "get",
				url: g_config.ajaxUrl + "log_h.jsp?cmd=dog&dogId=" + e.encodeUrl(k) + "&dogSrc=" + e.encodeUrl(h) + g,
			})
		};
...
HdGame.appendMusicIcon = function() {
	...
	if (g_config.style == 55) {
		...
		if (g_config.style == 49 || g_config.style == 69) {
			$(".body").append('<div class="' + (_manage ? "" : "soundIconOff ") + "soundIcon" + (hg.sound.list[0].optFlag != 1 ? "" : " soundIconHide") + '" style="z-index:700"></div>');
			...
			if (g_config.style == 67) {
				$(".myCutDownPriceList").append('<div class="' + (_manage ? "" : "soundIconOff ") + "soundIcon" + (hg.sound.list[0].optFlag != 1 ? "" : " soundIconHide") + '" style="z-index:700"></div>');
...
HdGame.getIsOutofJoinNumFlag = function(a) {
	//$.ajax({
	//	type: "post",
	//	url: g_config.ajaxUrl + "hdgame_h.jsp?cmd=checkJoinNum&gameId=" + g_config.gameId + "&openId=" + g_config.openId,
	//	success: function(c) {
	//		var b = $.parseJSON(c);
	//		HdGame.tlog("isOutofJoinNum", b);
